...
WITH CHECK (
    secure_access_token IS NOT NULL
    AND length(btrim(secure_access_token)) >= 12
);
...
-- (20261010) Giao dich thanh toan CHI admin duoc ghi. Webhook that (payOS/bank) phai chay
-- bang service_role - role nay bo qua RLS nen khong can policy cho anon.
CREATE POLICY "Admins can record payment transaction"
