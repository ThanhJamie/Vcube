...
USING (verified_status = 'Verified' OR public.is_admin() OR auth.uid() = user_id);
...
USING (
    EXISTS (
        ...
        WHERE wm.id = material_inventory_logs.material_id AND (wp.user_id = auth.uid() OR public.is_admin())
    )
    OR public.is_admin()
);
