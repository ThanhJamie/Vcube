...
CREATE POLICY "Public can view published products" ON public.products FOR SELECT TO anon, authenticated USING (status IN ('published','Published'));
...
-- (20261010) KHONG mo SELECT cong khai cho orders: chi chu don qua RPC token hoac admin.
DROP POLICY IF EXISTS "Public can insert orders" ON public.orders;
CREATE POLICY "Public can insert orders" ON public.orders FOR INSERT TO anon, authenticated WITH CHECK (secure_access_token IS NOT NULL AND length(btrim(secure_access_token)) >= 12);
DROP POLICY IF EXISTS "Admins can update orders" ON public.orders;
CREATE POLICY "Admins can update orders" ON public.orders FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
-- (20261010) KHONG mo SELECT cong khai cho user_profiles: chi chu so huu hoac admin.
DROP POLICY IF EXISTS "Admins can manage profiles" ON public.user_profiles;
CREATE POLICY "Admins can manage profiles" ON public.user_profiles FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Admins can manage materials" ON public.materials FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Admins can manage printer fleet" ON public.printer_fleet FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Admins can manage pricing config" ON public.pricing_config FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
