...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getRoleBadge = () => {
    ...
    if (role === 'lab') {
      return { text: 'LAB HUB', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' };
    }
    ...
    <>
      ...
      <header className="sticky top-0 z-40 bg-[#F8F9FF]/95 backdrop-blur-md border-b border-[#C5C6CD] px-4 sm:px-6 md:px-12 py-3 transition-all">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-3 sm:gap-6">
          ...
          {/* Zone 2: Navigation Links - ONLY displayed when logged in */}
          {isLoggedIn && (
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            {/* Quick Search on Desktop (only when logged in) */}
            {isLoggedIn && (
            ...
            {/* Cart Icon & Counter (only when logged in) */}
            {isLoggedIn && (
              <button
            ...
            {isLoggedIn ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 p-1 border border-[#C5C6CD] hover:border-[#00687A] rounded transition-all focus-visible:outline-none touch-target-btn cursor-pointer"
                  aria-label="Tài khoản người dùng"
                >
                  <div className={`w-7 h-7 flex items-center justify-center font-bold text-xs text-white rounded-xs ${
                    role === 'admin' ? 'bg-purple-900' : role === 'designer' ? 'bg-amber-700' : 'bg-[#091426]'
                  }`}>
                  ...
                  >
                    <div className="px-4 py-2 border-b border-[#E5EEFF] bg-[#F8FAFC]">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-tech text-[9px] uppercase tracking-widest text-[#00687A] font-bold">
                          {language === 'vi' ? 'TÀI KHOẢN ĐÃ ĐĂNG NHẬP' : 'LOGGED IN ACCOUNT'}
                        </span>
                        <span className={`text-[8px] font-tech font-bold px-1.5 py-0.5 rounded uppercase ${badge.color}`}>
                          {role}
                        </span>
                      </div>
                      <p className="font-bold text-sm text-[#091426] truncate">{profile?.displayName || user?.email?.split('@')[0] || 'Tài khoản'}</p>
                      <p className="text-[11px] font-tech text-[#545F73] truncate">{profile?.email || user?.email || ''}</p>
                    </div>
                    ...
                    <div className="py-1.5 divide-y divide-[#F8FAFC]">
                      <button
                        ...
                        className="w-full text-left px-4 py-2 hover:bg-[#EFF4FF] flex items-center gap-2 text-[#091426] transition-colors cursor-pointer"
                      >
                        <span className="material-symbols-outlined text-base text-[#545F73]">receipt_long</span>
                        <span>{t('myOrdersTracking', 'Đơn hàng & Theo dõi in', 'Orders & Print Tracking')}</span>
                      </button>
                      {(role === 'designer' || role === 'admin') && (
                        <button
                          onClick={() => {
                            setUserDropdownOpen(false);
                            handleNavClick('designer');
                          }}
                          className="w-full text-left px-4 py-2 hover:bg-[#EFF4FF] flex items-center justify-between text-[#091426] font-bold transition-colors cursor-pointer"
                        >
                          <span className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-base text-amber-700">design_services</span>
                            Studio Tác Giả 3D
                          </span>
                          <span className="text-[9px] font-tech px-1.5 py-0.5 bg-amber-100 text-amber-800 rounded font-bold">
                            CREATOR
                          </span>
                        </button>
                      )}
                      {role === 'admin' && (
                        ...
                        >
                          ...
                          <span className="text-[9px] font-tech px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded font-bold">
                            FORGE
                          </span>
                        </button>
                      )}
                    ...
                    <div className="pt-2 border-t border-[#E5EEFF] px-4 space-y-1.5">
                      <button
                        ...
                        className="w-full text-center py-1.5 bg-[#00687A] text-white text-xs font-bold uppercase rounded hover:bg-[#005463] transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="w-full text-center py-1.5 text-xs text-rose-600 hover:text-white hover:bg-rose-600 border border-rose-200 rounded font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                      >
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <Link
                  to="/auth/login"
                  className="px-2.5 sm:px-3.5 py-1.5 text-xs font-semibold text-[#091426] hover:text-[#00687A] hover:bg-[#EFF4FF] rounded border border-[#C5C6CD] transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs"
                >
                ...
                <Link
                  to="/auth/register"
                  className="px-2.5 sm:px-3.5 py-1.5 text-xs font-semibold text-white bg-[#091426] hover:bg-[#00687A] rounded transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
                >
      ...
      {mobileMenuOpen && (
        <>
          ...
          <div className="mobile-nav-panel p-6 space-y-6 bg-white">
            ...
            {/* Nav Items List - only displayed when logged in */}
            {isLoggedIn && (
              ...
              {isLoggedIn ? (
                <>
                  <button
                    ...
                    className="w-full py-2.5 bg-[#00687A] hover:bg-[#005463] text-white text-xs font-bold uppercase rounded text-center block transition-colors cursor-pointer"
                  >
                    {t('switchAccount', 'Tài Khoản & Thông Tin', 'Account & Details')}
                  </button>
                  <button
                    ...
                    className="w-full py-2.5 border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold uppercase rounded text-center flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-sm">logout</span>
                    <span>{t('signOut', 'Đăng Xuất Khỏi Thiết Bị', 'Sign Out')}</span>
                  </button>
