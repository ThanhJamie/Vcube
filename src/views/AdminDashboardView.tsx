...
import { AdminCostRulesPanel } from '../components/admin/AdminCostRulesPanel';
...
import { WorkshopEstimatorBOM } from '../components/admin/WorkshopEstimatorBOM';
...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({
  ...
  const validSections: AdminNavSection[] = [
    ...
    'cost-rules', 'materials', 'hardware', 'quote-calc', 'storefront', 'settings'
  ];
  ...
  const sectionBreadcrumbs: Record<AdminNavSection, { group: string; title: string; icon: string }> = {
    ...
    'cost-rules': {
      ...
      title: isVi ? 'Công Thức Tính Giá Inkiri' : 'Inkiri Cost Rules',
      icon: 'calculate'
    },
    materials: {
      group: isVi ? 'ĐỊNH GIÁ & BÁO GIÁ' : 'PRICING',
        ...
        <main className="p-4 sm:p-6 md:p-8 flex-1 max-w-7xl w-full mx-auto">
          ...
          {activeSection === 'cost-rules' && (
            <AdminCostRulesPanel
              ...
              onUpdateConfig={onUpdatePricingConfig}
              onShowToast={onShowToast}
            />
          )}
