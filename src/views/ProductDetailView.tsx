...
export const ProductDetailView: React.FC<ProductDetailViewProps> = ({
  ...
  const selectedMaterialObj = MATERIALS_CATALOG.find(m => m.name.toLowerCase().includes(selectedMaterial.toLowerCase())) || MATERIALS_CATALOG[0];
  const dynamicPricePhysical = Math.round(product.pricePhysical * (selectedMaterialObj?.unitPriceMultiplier || 1));
